...
tables in the \CodeIn{from} clause, columns in the \CodeIn{select} clauses.
...
%their tasks.
a GUI often do not permit users to personalize
...
a practical programming language (even a high-level domain
...
In this work, we present an  approach for synthesizing a SQL query
from input-output examples. In particular, our approach takes an example input
table(s) and output table from the end-users, and then automatically
infers a SQL query (or multiple queries, if existed) that implements
a transformation that satisfies the example. If the SQL query is applied
...
It includes the following two major steps:
...
the most likely program to the user, if multiple solutions exist.

\vspace{1mm}
\noindent \textbf{\textit{Supported SQL Subset.}}
The full SQL 93 specification contains over 1000 pages, and it is
impractical and unnecessary to support all language features.
...
During our study of end-user's real problems from online help forums, we also carefully studied what would
...
The supported subset, though far from complete, is expressive
...
the example input and output, and then \textit{guesses} what a desirable
SQL query may look like. The output of this step is a set of all possible \textit{incomplete} SQL
query skeletons, each of which contains unfilled parts. The second step,
called \textit{SQL Query Completion} takes as inputs the created
SQL skeletons, fulfills unknown parts with possible answers, and then produces a set of
syntactically-valid SQL queries. In particular, when performing
SQL query completion, our technique uses a decision-tree learning algorithm~\cite{}
(Section~\ref{}) to infer the conditions, and uses 
type-directed search (Section~\ref{}) to infer possible aggregates in a query.

\vspace{1mm}
\noindent \textbf{\textit{Query Candidate Ranking Strategy.}}
Input-output examples provided by end-users are likely to
be under-specififed. It is entirely possible that multiple
queries can be synthensized to satisfy the provided examples.
...
Thus, we evalaute our technique in two ways. First, we pick up 6 SQL
exercises from a classic database textbook~\cite{cowbook}. SQL exercises
from a database textbook covers a large part of practically useful SQL constructs
a constructor wishes students to master, and can be served as golden test
to evaluate the expressiveness of our supported SQL subset. Second,
we chose 5 non-trival SQL problems rasied by real end-users on online
help forums, and test whether our technique can effectively infer desirable
SQL queries.

As a result, our technique successfully synthesizes 5 out of 6
textbook exercises and solves all 5 forum problems, with time cost of
1 minute per each exercise (or problem).
...
\item \textbf{Tool.} A practical tool that implements the proposed approach (Section~\ref{sec:implementation}).
