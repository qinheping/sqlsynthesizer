...
\e ::= {} & \ce \;\;|\ \texttt{COUNT}(\column) \;\;|\ \texttt{COUNT}(\texttt{DISTINCT}\;\column)\\
    & |\ \texttt{MIN}(\column) \;\;|\ \texttt{MAX}(\column) \\
    & |\ \texttt{SUM}(\column) \;\;|\ \texttt{AVG}(\column) 
\end{align*}
\normalsize%
...
expressiveness. For example, the \CodeIn{BETWEEN}
...
Among the 12 received responses, the average rating of the proposed SQL subset is 4.5. Most of
