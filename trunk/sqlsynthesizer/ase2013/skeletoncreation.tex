...
\vspace{1mm}
%\noindent
{\textbf{Determining join conditions. }} Instead
of enumerating all possible ways to join query tables,
which may lead to a large number of join conditions,
\ourtool uses two rules (below) to identify
the most likely join conditions.
\ourtool repeatedly applies these two rules
to join two different tables from the query tables
until all tables get joined.
%, and outputs \textit{all} satisfied join conditions. \todo{xx}
...
For example, suppose the \CodeIn{student\_id} column 
in the \CodeIn{student} table of Figure~\ref{fig:motivating}(a)
is renamed to \CodeIn{student\_key}, \ourtool will no longer
find a column with the same name and data type in
table \CodeIn{student} and table \CodeIn{enrolled}. If so,
...
and \CodeIn{student\_key = score}; and create
...
a column with the same name exists in an input table.
If so, \ourtool uses the first matched column from the input
table as the projection column.
Otherwise, the column in the output table must be created by using an aggregate.
...
in Figure~\ref{fig:motivating}. The missing parts (between $<$ and $>$) will
