...
includes all tuples containing the output table; and the negative part includes the rest
tuples.

The standard way for rule learning is using a decision-tree-based
algorithm. However, the key challenge is how
to design a good feature set for effective learning.
...
them together with the existing tuple values
for rule learning.
...
\begin{figure*}[t]
  ...
  with additional features (Right). For brevity, only relevant
  aggregation features are shown. Using the added aggregation
  features, \ourtool infers two query conditions that
  transform the original table into the table shown in (b),
  which contains the output table shown in (c).
  Without the aggregation features,
  \ourtool will \textit{fail} to learn the above conditions.
...
\ourtool divides the learned conditions into two disjoint parts,
...
every input table column; and check whether
