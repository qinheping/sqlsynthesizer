...
examples, all examples are written by a different graduate
student (whose research field is not database-related) from University of Washington other than
\ourtool's developers.

We checked \ourtool's correctness by comparing its
output with the expected SQL queries. Specifically,
for textbook exercises, we compared \ourtool's output with
their correct answers; for forum questions, we
manually wrote the correct SQL query and then determined
...
interactions until \ourtool synthesized a desirable SQL query.
...
due to some unsupported features, such as
...
one more tuple in the input table, such as "Chris, 3"
(a tuple with value ``Chris'' in the name column and ``3''
