...
to infer SQL queries from examples.
As we have discussed thoroughly in Section~\ref{sec:comparison},
QBO is limited in inferring simple select-project-join queries,
while excluding many useful SQL features, such as aggregates,
the \CodeIn{HAVING} clause, and the \CodeIn{GROUP BY}
clause. Our experiments also confirmed that QBO is not
sufficient to infer queries for many use-case scenarios.
