...
write a correct database query (i.e., a SQL query) even after
receiving step-by-step, detailed,
and syntactically correct instructions.
Thus, typical end-users often need to
...
from input-output examples\footnote{
The SQL queries in this paper refer to the read-only
database queries, which do not modify the database content.}.
...
to describe \textit{what} the task is and is more
natural for a typical end-user to use.
...
to satisfy the given input-output examples,
...
and it needs to project data on certain columns as the output.
All such operations must be handled properly.% This requires \todo{new challenges}
%unlike a spreadsheet macro  that
%re-shapes one input table to one output table~\cite{Harris:2011},


To make SQL query synthesis from examples feasible in practice,
...
identify the table joins and projection columns in
...
the \CodeIn{ORDER BY} clause; and
outputs a list of syntactically-valid candidate
queries. 
